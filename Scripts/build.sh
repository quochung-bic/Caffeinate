...
# --no-xcode is unavailable here, and the empty SPM_PRODUCT is what says so.
# Every SwiftUI view lives in the Caffeinate/ application target rather than in
# CaffeinateKit, which holds no GUI at all by design, so there is nothing for
# SwiftPM to link an executable from. Supporting it would mean moving the whole
# interface into the package — a different architecture, not a build flag.
readonly SPM_PRODUCT=""
readonly INFO_PLIST=""
readonly APP_ICON=""
readonly NO_XCODE_REASON="Every view lives in the Caffeinate application target, and
    CaffeinateKit holds no GUI, so SwiftPM has nothing to build an app from.
    This one needs Xcode."
